                    ...
                    <form action="{{ route('admin.users.update', $user) }}" method="POST" class="max-w-xl">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="is_admin" id="is_admin" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                                <option value="0" {{ old('is_admin', $user->is_admin) == '0' ? 'selected' : '' }}>User (Regular Account)</option>
                                <option value="1" {{ old('is_admin', $user->is_admin) == '1' ? 'selected' : '' }}>Admin (Full Access)</option>
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="is_active" id="is_active" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                                <option value="1" {{ old('is_active', $user->is_active) == '1' ? 'selected' : '' }}>Enabled</option>
                                <option value="0" {{ old('is_active', $user->is_active) == '0' ? 'selected' : '' }}>Disabled</option>
                            </select>
